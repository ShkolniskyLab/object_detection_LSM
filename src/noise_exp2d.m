function [noise,P,S,T1]=noise_exp2d(N,K,refpsd,gpu_use)
...
if refpsd
    ...
    if gpu_use == 1
        M = gpuArray(M);
        omega0 = gpuArray(omega0);
        omega1= gpuArray(omega1);
        omega_x= gpuArray(omega_x);
        omega_y= gpuArray(omega_y);
        c= gpuArray(c);
        S= gpuArray(S);
    end
    for j1=-100:100
