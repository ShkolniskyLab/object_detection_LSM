function [z_tilde,S_z] = projected_noise_simulation_from_noise_patches_z_tilde(noise_samples,basis,num_of_exp_noise)
...
%     noise_samples = gpuArray(noise_samples);
    sz = sqrt(size(noise_samples,1));
    sz_pn = size(conv2(reshape(noise_samples(:,1),sz,sz),flip(flip(basis(:,:,1),1),2),'valid'),1);
    ...
    parfor i=1:num_of_exp_noise
        ...
        noise_img = reshape(gpuArray(noise_samples(:,i)'),sz,sz);
        for j = 1:size(basis,3)
            S(:,:,j) =conv2(noise_img,flip(flip(basis(:,:,j),1),2),'valid').^2;
        end
